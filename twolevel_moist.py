...
class TwoLevel(object):
    ...
    def gettend(self,vrtspec,divspec,thetaspec):
        ...
        if self.moistfact < 1:
            vadvtheta = 0.5*self.delth*divg
            vadvtheta = np.where(divg > 0, self.moistfact*vadvtheta, vadvtheta)
            # need to remove global mean or global mean temp will increase
        ...
        (self.thetarefspec-thetaspec)/self.tdiab - vadvspec
...
if __name__ == "__main__":
    ...
    w = model.dp*sp.spectogrd(divspec)
    ...
    im=m.imshow(w,cmap=plt.cm.RdBu_r,vmin=-2,vmax=2,interpolation="nearest")
    ...
    def updatefig(*args):
        ...
        for n in range(nout):
            vrtspec, divspec, thetaspec = model.rk4step(vrtspec, divspec, thetaspec)
        w = model.dp*sp.spectogrd(divspec)
        w, lonstmp = addcyclic(w, lons1d_save)
        im.set_data(w)
        txt.set_text('W Day %10.2f' % \
                     float(model.t/86400.))
        return im,txt
